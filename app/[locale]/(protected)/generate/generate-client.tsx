...
export default function GenerateClientPage({
  ...
  featuredTemplates: PublicTemplate[];
}) {
  ...
  useEffect(() => {
    ...
    if (matched) {
      setSelectedTemplateSlug(matched.slug);
      setGenerationError(null);
    } else {
      // Template not found in current registry — show error
      setGenerationError(t("errors.templateNotAvailable"));
    }
  }, [templates, t]);
  ...
  const dynastyTemplates = useMemo(() => {
    return DYNASTY_TABS.map((dynasty) => ({
      dynasty,
      templates: templates.filter((t) => t.dynasty === dynasty),
                ...
                <div className="rounded-[24px] border border-[rgba(255,247,236,0.08)] bg-[rgba(17,17,20,0.90)] p-5">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <span className="text-right font-medium text-[#E8C27A]">{templateDisplayName}</span>
                    </div>
                    <div className="flex items-center justify-between gap-4">
                  ...
                  {hasTemplates ? (
                    <>
                      <div className="flex flex-wrap gap-2">
                        {dynastyTemplates.map(({ dynasty }) => {
                          const isSelected = activeDynasty === dynasty || (!activeDynasty && dynasty === dynastyTemplates[0]?.dynasty);
                          return (
                            <button
                              key={dynasty}
                              type="button"
                              onClick={() => {
                                ...
                                const firstInDynasty = templates.find((t) => t.dynasty === dynasty);
                                if (firstInDynasty) {
                                  setSelectedTemplateSlug(firstInDynasty.slug);
                                  setGenerationError(null);
                                }
                              }}
                      ...
                      <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
                        {templates.filter((t) => t.dynasty === (activeDynasty || dynastyTemplates[0]?.dynasty)).map((template) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="px-4 pb-4 pt-3">
                          <div className="mb-2 inline-flex rounded-full border border-[rgba(232,194,122,0.16)] px-2 py-0.5 text-[11px] text-[#E8C27A]">{template.dynasty}</div>
                          <p className="truncate text-base font-semibold text-[rgba(255,247,236,0.92)]">{template.name.zh || template.name.en}</p>
                        </div>
                        {isSelected && (
